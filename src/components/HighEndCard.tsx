...
const HighEndCard = ({ title, description, whatsappMessage }: HighEndCardProps) => {
  const encodedMessage = encodeURIComponent(whatsappMessage);
  const href = `${WHATSAPP_BASE}?text=${encodedMessage}`;

  return (
    <div className="bg-background p-6 md:p-8 rounded-xl border border-primary/30 transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 group">
